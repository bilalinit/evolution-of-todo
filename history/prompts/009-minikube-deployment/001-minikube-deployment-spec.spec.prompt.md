...
LINKS:
  ...
  PR: null
FILES_YAML:
  - specs/009-minikube-deployment/spec.md
  - specs/009-minikube-deployment/checklists/requirements.md
TESTS_YAML: []
OUTCOME: Specification created with all mandatory sections completed, validated, and passed quality checks
EVALUATION: PASSED - Spec is technology-agnostic, all requirements testable, success criteria measurable
---
...
```text
name the new branch "009-minikube-deployment" here are the specs:
...
- **FR-001**: System MUST support deployment of phase-4 frontend (Next.js 16+) to Minikube
- **FR-002**: System MUST support deployment of phase-4 backend (FastAPI) to Minikube
- **FR-003**: System MUST use multi-stage Docker builds for both frontend and backend
- **FR-004**: System MUST create .dockerignore files to exclude unnecessary files from builds
...
- **FR-008**: System MUST require `eval $(minikube docker-env)` before building images
- **FR-009**: System MUST use `imagePullPolicy: IfNotPresent` for local builds
...
- **FR-012**: System MUST support external Neon PostgreSQL database connection
...
- **FR-016**: System MUST document the `minikube tunnel` requirement for LoadBalancer access
- **FR-017**: System MUST include environment variable configuration via Helm values
- **FR-018**: System MUST support CORS configuration for frontend-backend communication
- **FR-019**: Frontend MUST configure `NEXT_PUBLIC_API_URL` to use backend service name
- **FR-020**: Backend MUST accept connections from Kubernetes internal network
- **FR-021**: Helm charts MUST be created in `phase-4/helm-charts/` directory (at `phase-4/helm-charts/frontend/` and `phase-4/helm-charts/backend/`)
...
- **Docker Image**: Container image built in Minikube's Docker environment
...
```dockerfile
# Multi-stage build for Next.js 16+ standalone mode
...
| Frontend Variable | Description | Source |
|-------------------|-------------|--------|
| NEXT_PUBLIC_API_URL | Backend API URL | Service DNS: `http://backend:8000` |
| NEXT_PUBLIC_DEMO_MODE | Demo mode flag | values.yaml |
| NEXT_PUBLIC_DEBUG | Debug mode flag | values.yaml |
| BETTER_AUTH_SECRET | Auth secret | Kubernetes Secret |

| Backend Variable | Description | Source |
|-------------------|-------------|--------|
| DATABASE_URL | PostgreSQL connection | Kubernetes Secret (Neon) |
| BETTER_AUTH_SECRET | Auth secret | Kubernetes Secret |
| OPENAI_API_KEY | OpenAI API key | Kubernetes Secret |
| XIAOMI_API_KEY | Xiaomi API key | Kubernetes Secret |
| HOST | Server host | values.yaml (0.0.0.0) |
| PORT | Server port | values.yaml (8000) |
| DEBUG | Debug mode | values.yaml |
...
│             │                               │                   │
│  ┌──────────▼───────────┐       ┌───────────▼──────────┐       │
│  │  LoadBalancer Svc    │       │   ClusterIP Svc      │       │
...
│             ▼                                                   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │              External Access (10.96.x.x:3000)            │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│                          ┌──────────────┐                        │
│                          │  PostgreSQL  │                        │
...
| Configuration | Dev | Staging | Production-Like |
|---------------|-----|---------|-----------------|
...
- **SC-007**: Database operations (CRUD) persist to external Neon correctly
...
- **TV-001**: Docker images build successfully in Minikube Docker environment
- **TV-002**: All Kubernetes resources are created without errors
- **TV-003**: LoadBalancer services receive external IPs via `minikube tunnel`
...
- **TV-007**: Database SSL connections work with external Neon
...
- **External Neon Database**: Existing PostgreSQL database with SSL

### Internal Dependencies

- **phase-4/frontend**: Next.js 16+ application with ChatKit integration
- **phase-4/backend**: FastAPI application with OpenAI Agents SDK
...
|------|------------|--------|------------|
| Developer skips `eval $(minikube docker-env)` | High | High | Clear documentation and error checks |
...
| `minikube tunnel` confusion | High | Medium | Step-by-step access instructions |
...
- [ ] Create phase-4/helm-charts/frontend chart
- [ ] Create phase-4/helm-charts/backend chart
- [ ] Configure values.yaml for dev stage
- [ ] Configure values.yaml for staging stage
- [ ] Configure values.yaml for production-like stage
...
- ChatKit Skill: `.claude/skills/chatkit/`
- OpenAI Agents SDK Skill: `.claude/skills/openai-agents-sdk/`
```

## Assistant Response (Summary)

Successfully created feature specification for Minikube deployment:

**Branch Created**: `009-minikube-deployment`
**Spec File**: `specs/009-minikube-deployment/spec.md`

**Key Outcomes**:
- 4 prioritized user stories (P1-P3) covering local dev, staging, production-like, and rollback scenarios
- 21 functional requirements covering deployment, configuration, security, and operations
- 10 measurable success criteria focusing on developer productivity and application reliability
- Comprehensive edge cases, dependencies, and risk analysis
...
1. `specs/009-minikube-deployment/spec.md` - Complete feature specification
2. `specs/009-minikube-deployment/checklists/requirements.md` - Quality validation checklist (PASSED)
3. `history/prompts/009-minikube-deployment/001-minikube-deployment-spec.spec.prompt.md` - This PHR
