...
user: user
command: /sp.plan
labels: ["chatkit", "integration", "planning", "openai", "mcp"]
links:
...
```text
# Web application (current structure maintained)
...
I've created a comprehensive implementation plan for ChatKit integration with your existing Next.js 16 + FastAPI application. The plan includes:

### ✅ **Key Achievements**

1. **Phase 0: Research Complete** - Resolved all technical unknowns:
   - Confirmed ChatKit requires OPENAI_API_KEY for session management (even with other providers)
   - Verified Next.js 16+ App Router compatibility patterns
   - Validated MCP tool integration with ChatKit
   - Confirmed Better Auth integration via httpOnly cookies

2. **Phase 1: Design Complete** - Created detailed architecture:
   - **Data Model**: PostgreSQL schema extensions for ChatKit threads with user isolation
   - **API Contracts**: Session creation/refresh endpoints with JWT validation
   - **Backend Architecture**: ChatKitServer and ChatKitStore implementations
   - **Frontend Pattern**: ChatKit widget with context injection and localStorage persistence

3. **Documentation Created**:
   - `research.md` - Technical compatibility analysis
   - `data-model.md` - Database schema and migration strategy
   - `quickstart.md` - Step-by-step setup guide
   - `contracts/session-api.md` - Complete API documentation

### 🎯 **Critical Requirements Identified**

1. **OPENAI_API_KEY is REQUIRED** - Even when using other providers (Xiaomi mimo-v2-flash)
2. **CDN Script Loading** - Must be in HTML body, not head, for Next.js 16+
3. **User Isolation** - All queries must include user ID filtering
4. **Session Management** - Token-based auth with httpOnly cookies

### 📋 **Next Actions**

The plan is ready for implementation. Key next steps:

1. **Run `/sp.tasks`** to generate detailed implementation tasks
2. **Add Dependencies**: `uv add openai` (backend) + `npm install @openai/chatkit-react` (frontend)
3. **Set Environment Variables**: Add `OPENAI_API_KEY` to backend
4. **Create Database Tables**: Run migration for `chatkit_thread` and `chatkit_thread_item`
5. **Implement Backend**: Create `chatkit_server.py` and `chatkit_store.py`
6. **Implement Frontend**: Add ChatKit CDN and create `ChatKitWidget.tsx`

### 🚨 **Risk Mitigation**

- **OpenAI API Key**: Documented as CRITICAL requirement with validation
- **MCP Tools**: Existing `/api/chat` endpoint kept as fallback
- **CDN Loading**: Enhanced detection using `customElements.whenDefined()`
- **Thread Persistence**: localStorage + PostgreSQL hybrid approach

### ✅ **Constitution Compliance**

- ✅ **Simplicity**: ChatKit reduces frontend complexity
- ✅ **Single Source of Truth**: PostgreSQL remains primary database
- ✅ **No Over-engineering**: Additive changes only
- ✅ **Smallest Viable Change**: Backend changes are minimal
- ✅ **Testable**: Clear acceptance criteria for each integration point

The implementation plan is comprehensive, technically sound, and ready for execution. All architectural decisions are documented with clear rationale and alternatives considered.

## Outcome

- ✅ Impact: Complete ChatKit integration plan created with 3 phases of implementation
- 🧪 Tests: Acceptance criteria defined for backend, frontend, and integration testing
- 📁 Files: 5 documentation files created (plan, research, data-model, quickstart, contracts)
- 🔁 Next prompts: Ready for `/sp.tasks` to generate detailed implementation tasks
- 🧠 Reflection: All technical unknowns resolved through research; plan addresses constitution requirements
