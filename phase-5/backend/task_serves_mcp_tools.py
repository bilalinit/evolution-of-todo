...
Phase 5: Added support for recurring tasks, reminders, and tags.
"""
import sys
...
async def create_task(
    ...
    user_timezone_offset_hours: Optional[float] = None
) -> dict:
    """
    ...
    Args:
        ...
        recurring_end_date: Optional end date for recurrence in YYYY-MM-DD format
        reminder_at: Optional reminder datetime in LOCAL time format YYYY-MM-DDTHH:MM:SS
        tags: Optional comma-separated list of tags (e.g., "urgent,work,frontend")
        user_timezone_offset_hours: User's timezone offset from UTC (e.g., +5.0 for PKT, -5.0 for EST)
    ...
    try:
        async with async_session_factory() as session:
            ...
            recurring_end_date_obj = None
            if recurring_end_date:
                recurring_end_date_obj = datetime.fromisoformat(recurring_end_date)

            # Parse reminder datetime if provided and convert from local to UTC
            reminder_at_obj = None
            if reminder_at:
                local_dt = datetime.fromisoformat(reminder_at)
                # If user provided timezone offset, convert local time to UTC
                if user_timezone_offset_hours is not None:
                    from datetime import timedelta
                    # Subtract the offset to convert local to UTC
                    reminder_at_obj = local_dt - timedelta(hours=user_timezone_offset_hours)
                else:
                    # Assume already UTC if no offset provided
                    reminder_at_obj = local_dt
            ...
            result = await service.create(
                ...
                recurring_end_date=recurring_end_date_obj,
...
async def update_task(
    ...
    Args:
        ...
        reminder_at: New reminder datetime in LOCAL time (optional)
        tags: New comma-separated list of tags (optional)
        user_timezone_offset_hours: User's timezone offset from UTC (e.g., +5.0 for PKT)
