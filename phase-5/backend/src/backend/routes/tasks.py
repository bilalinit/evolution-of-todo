"""
Task routes for CRUD operations.
"""
from typing import Optional
from fastapi import APIRouter, Depends, HTTPException, Query, status
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select, and_, or_
from uuid import UUID
from datetime import datetime, timezone

from backend.database import get_session
from backend.middleware.auth import get_current_user, verify_user_ownership
from backend.models.task import (
    Task, TaskCreate, TaskUpdate, TaskListResponse, TaskDetailResponse,
    Priority, Category
)
from backend.models.notification import Notification
from backend.models.audit_log import EventType
from backend.services.audit_service import AuditService
from backend.utils.event_publisher import (
    publish_task_created,
    publish_task_updated,
    publish_task_completed,
    publish_task_deleted,
)

router = APIRouter()


def make_naive(dt: Optional[datetime]) -> Optional[datetime]:
    """Strip timezone info from datetime for database storage (TIMESTAMP WITHOUT TIME ZONE)."""
    return dt.replace(tzinfo=None) if dt and dt.tzinfo else dt


@router.get("/tasks", response_model=TaskListResponse)
async def get_tasks(
    user_id: str,
    current_user: str = Depends(get_current_user),
    session: AsyncSession = Depends(get_session),
    # Query parameters for filtering
    status: Optional[str] = Query(None, description="Filter by status: pending or completed"),
    priority: Optional[Priority] = Query(None, description="Filter by priority"),
    category: Optional[Category] = Query(None, description="Filter by category"),
    search: Optional[str] = Query(None, description="Search in title and description"),
    tags: Optional[str] = Query(None, description="Filter by tags (comma-separated)"),
    sort_by: Optional[str] = Query("created_at", description="Sort field: created_at, due_date, priority, title"),
    order: Optional[str] = Query("desc", description="Sort order: asc or desc")
):
    """
    Get all tasks for a user with optional filtering and sorting.

    Requires authentication and verifies user ownership.
    """
    # Verify user ownership
    await verify_user_ownership(user_id, current_user)

    # Build query
    query = select(Task).where(Task.user_id == user_id)

    # Apply filters
    if status:
        completed = status.lower() == "completed"
        query = query.where(Task.completed == completed)

    if priority:
        query = query.where(Task.priority == priority)

    if category:
        query = query.where(Task.category == category)

    if tags:
        # Filter by tags using JSONB containment operator
        tag_list = [tag.strip() for tag in tags.split(',') if tag.strip()]
        if tag_list:
            # Use @> operator to check if tags array contains any of the specified tags
            tag_conditions = [Task.tags.op('@>')(text(f"'[{tag}]'::jsonb")) for tag in tag_list]
            query = query.where(or_(*tag_conditions))

    if search:
        search_pattern = f"%{search}%"
        # Build search conditions
        search_conditions = [
            Task.title.ilike(search_pattern),
            Task.description.ilike(search_pattern),
        ]
        # Add tags search - cast tags to text for searching
        search_conditions.append(Task.tags.astext.ilike(search_pattern))
        query = query.where(or_(*search_conditions))

    # Apply sorting
    sort_field = getattr(Task, sort_by, Task.created_at)
    if order.lower() == "asc":
        query = query.order_by(sort_field.asc())
    else:
        query = query.order_by(sort_field.desc())

    # Execute query
    result = await session.execute(query)
    tasks = result.scalars().all()

    return TaskListResponse.from_tasks(tasks)


@router.get("/tasks/{task_id}", response_model=TaskDetailResponse)
async def get_task(
    user_id: str,
    task_id: UUID,
    current_user: str = Depends(get_current_user),
    session: AsyncSession = Depends(get_session)
):
    """
    Get a specific task by ID.

    Requires authentication and verifies user ownership.
    """
    # Verify user ownership
    await verify_user_ownership(user_id, current_user)

    # Query specific task
    query = select(Task).where(
        and_(
            Task.id == task_id,
            Task.user_id == user_id
        )
    )
    result = await session.execute(query)
    task = result.scalar_one_or_none()

    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Task not found"
        )

    return TaskDetailResponse.from_task(task)


@router.post("/tasks", response_model=TaskDetailResponse, status_code=status.HTTP_201_CREATED)
async def create_task(
    user_id: str,
    task_data: TaskCreate,
    current_user: str = Depends(get_current_user),
    session: AsyncSession = Depends(get_session)
):
    """
    Create a new task for the authenticated user.

    Requires authentication and verifies user ownership.
    """
    # Verify user ownership
    await verify_user_ownership(user_id, current_user)

    # Create task model
    task = Task(
        title=task_data.title,
        description=task_data.description,
        priority=task_data.priority,
        category=task_data.category,
        due_date=task_data.due_date,
        user_id=user_id,
        recurring_rule=task_data.recurring_rule,
        recurring_end_date=make_naive(task_data.recurring_end_date),
        reminder_at=make_naive(task_data.reminder_at),
        tags=task_data.tags or []
    )

    # Add to database
    session.add(task)
    await session.commit()
    await session.refresh(task)

    # Publish event for audit logging and WebSocket broadcast
    await publish_task_created(
        user_id=user_id,
        task_id=str(task.id),
        title=task.title,
        description=task.description,
        priority=task.priority.value,
        due_date=task.due_date.isoformat() if task.due_date else None,
        reminder_at=task.reminder_at.isoformat() if task.reminder_at else None,
        recurring_rule=task.recurring_rule,
        recurring_end_date=task.recurring_end_date.isoformat()
        if task.recurring_end_date else None,
        tags=task.tags or [],
    )

    return TaskDetailResponse.from_task(task)


@router.put("/tasks/{task_id}", response_model=TaskDetailResponse)
async def update_task(
    user_id: str,
    task_id: UUID,
    task_data: TaskUpdate,
    current_user: str = Depends(get_current_user),
    session: AsyncSession = Depends(get_session)
):
    """
    Update an existing task.

    Requires authentication and verifies user ownership.
    """
    # Verify user ownership
    await verify_user_ownership(user_id, current_user)

    # Get existing task
    query = select(Task).where(
        and_(
            Task.id == task_id,
            Task.user_id == user_id
        )
    )
    result = await session.execute(query)
    task = result.scalar_one_or_none()

    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Task not found"
        )

    # Store before state for audit
    before_state = task.to_dict()

    # Update fields
    if task_data.title is not None:
        task.title = task_data.title
    if task_data.description is not None:
        task.description = task_data.description
    if task_data.completed is not None:
        task.completed = task_data.completed
    if task_data.priority is not None:
        task.priority = task_data.priority
    if task_data.category is not None:
        task.category = task_data.category
    if task_data.due_date is not None:
        task.due_date = task_data.due_date
    if task_data.recurring_rule is not None:
        task.recurring_rule = task_data.recurring_rule
    if task_data.recurring_end_date is not None:
        task.recurring_end_date = make_naive(task_data.recurring_end_date)
    if task_data.reminder_at is not None:
        old_reminder = task.reminder_at
        task.reminder_at = make_naive(task_data.reminder_at)

        # If reminder changed to a future time, reset reminder_sent flag
        # and delete existing notification so it triggers again
        if old_reminder != task.reminder_at:
            now_utc = datetime.now(timezone.utc).replace(tzinfo=None)
            if task.reminder_at and task.reminder_at > now_utc:
                task.reminder_sent = False

                # Delete existing notification for this task
                existing_notif = await session.execute(
                    select(Notification).where(
                        and_(
                            Notification.task_id == task_id,
                            Notification.user_id == user_id
                        )
                    )
                )
                notif = existing_notif.scalar_one_or_none()
                if notif:
                    await session.delete(notif)
    if task_data.tags is not None:
        task.tags = task_data.tags

    # Update timestamp
    task.updated_at = datetime.utcnow()

    await session.commit()
    await session.refresh(task)

    # Publish event for audit logging and WebSocket broadcast
    await publish_task_updated(
        user_id=user_id,
        task_id=str(task.id),
        before=before_state,
        after=task.to_dict(),
    )
    
    # If task was just completed, also publish task-completed event for recurring task generation
    completed_before = before_state.get("completed", False)
    if not completed_before and task.completed:
        await publish_task_completed(
            user_id=user_id,
            task_id=str(task.id),
            title=task.title,
            recurring_rule=task.recurring_rule,
            recurring_end_date=task.recurring_end_date.isoformat()
            if task.recurring_end_date else None,
        )

    return TaskDetailResponse.from_task(task)


@router.patch("/tasks/{task_id}/complete", response_model=TaskDetailResponse)
async def toggle_complete(
    user_id: str,
    task_id: UUID,
    current_user: str = Depends(get_current_user),
    session: AsyncSession = Depends(get_session)
):
    """
    Toggle task completion status.

    For recurring tasks, completing will create the next instance.

    Requires authentication and verifies user ownership.
    """
    # Verify user ownership
    await verify_user_ownership(user_id, current_user)

    # Get existing task
    query = select(Task).where(
        and_(
            Task.id == task_id,
            Task.user_id == user_id
        )
    )
    result = await session.execute(query)
    task = result.scalar_one_or_none()

    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Task not found"
        )

    # Toggle completion
    is_completing = not task.completed
    task.completed = is_completing

    # Update timestamp
    task.updated_at = datetime.utcnow()

    # If completing a recurring task, create next instance
    # Note: This is now handled by recurring-service via event
    # We no longer create next instance directly here

    await session.commit()
    await session.refresh(task)

    # Publish event for audit logging, WebSocket broadcast, and recurring task generation
    await publish_task_completed(
        user_id=user_id,
        task_id=str(task.id),
        title=task.title,
        recurring_rule=task.recurring_rule,
        recurring_end_date=task.recurring_end_date.isoformat()
        if task.recurring_end_date else None,
    )

    return TaskDetailResponse.from_task(task)


@router.delete("/tasks/{task_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_task(
    user_id: str,
    task_id: UUID,
    current_user: str = Depends(get_current_user),
    session: AsyncSession = Depends(get_session)
):
    """
    Delete a task.

    Requires authentication and verifies user ownership.
    """
    # Verify user ownership
    await verify_user_ownership(user_id, current_user)

    # Get existing task
    query = select(Task).where(
        and_(
            Task.id == task_id,
            Task.user_id == user_id
        )
    )
    result = await session.execute(query)
    task = result.scalar_one_or_none()

    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Task not found"
        )

    # Store task snapshot for audit before deletion
    task_snapshot = task.to_dict()

    # Publish event before deletion
    await publish_task_deleted(
        user_id=user_id,
        task_id=str(task.id),
        title=task.title,
    )

    # Delete related notifications first (foreign key constraint)
    notifications_query = select(Notification).where(Notification.task_id == task.id)
    notifications_result = await session.execute(notifications_query)
    notifications = notifications_result.scalars().all()

    for notification in notifications:
        await session.delete(notification)

    # Delete task
    await session.delete(task)
    await session.commit()

    # No content returned for DELETE
    return