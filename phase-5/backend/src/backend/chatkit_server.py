...
class TodoChatKitServer(ChatKitServer):
    ...
    async def respond(
        ...
        server = MCPServerStdio(
            params={
                "command": "uv",
                "args": ["run", "--no-sync", "task_serves_mcp_tools.py"],
    ...
    def _enhance_input_with_context(self, agent_input: list, user_info: dict, metadata: dict) -> str:
        ...
        today_pkt = now_pkt.strftime("%A, %B %d, %Y")
        current_time_pkt = now_pkt.strftime("%A, %B %d, %Y at %I:%M %P")

        # Add user context prefix with current date in PKT
        ...
        if page_context:
            ...
            enhanced_input = f"{user_context} {page_info}\n\n{text_content}"
        else:
