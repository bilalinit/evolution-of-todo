...
export interface Task {
  ...
  recurring_rule?: RecurringRule;  // Recurrence rule
  recurring_end_date?: string;     // ISO 8601 timestamp for recurrence end
  parent_task_id?: string;         // Parent task UUID for recurring instances
  reminder_at?: string;            // ISO 8601 timestamp for reminder
...
/**
 * Parses a search query to extract tag filters (using #tag syntax)
 * @param query - The search query string
 * @returns Object with searchText (without tags) and tags array
 * @example
 * parseSearchQuery("meeting #work #urgent")
 * // Returns { searchText: "meeting", tags: ["work", "urgent"] }
 */
export function parseSearchQuery(query: string): {
  searchText: string;
  tags: string[];
} {
  // Extract #words as tags, rest as search text
...
export interface CreateTaskFormData {
  ...
  recurring_end_date?: string;  // Date string in YYYY-MM-DD format
  reminder_at?: string;         // Datetime string in YYYY-MM-DDTHH:MM format
