...
// Phase 5: Advanced Task Features API Functions
...
export async function getNotifications(
  ...
  const queryString = params.toString();
  const endpoint = `/api/${userId}/notifications${queryString ? `?${queryString}` : ''}`;
...
export async function markNotificationRead(
  ...
  return apiClient.post<{ success: boolean }>(`/api/${userId}/notifications/${notificationId}/read`, {});
}
...
export async function markAllNotificationsRead(userId: string): Promise<{ count: number }> {
  return apiClient.post<{ count: number }>(`/api/${userId}/notifications/read-all`, {});
}
...
export async function deleteNotification(
  ...
  return apiClient.delete<void>(`/api/${userId}/notifications/${notificationId}`);
}
...
export async function getAuditLogs(
  ...
  const queryString = params.toString();
  const endpoint = `/api/${userId}/audit${queryString ? `?${queryString}` : ''}`;

  return apiClient.get<AuditLogListResponse>(endpoint);
}